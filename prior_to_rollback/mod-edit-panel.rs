///typforge0.0.1\crates\typforge\src\editor\editor_panel\mod.rs
mod completions;
mod handlers;
mod render;
...
use typforge_core::intel::{get_completions, get_hover_info, Completion, Tooltip};
...
// --- NEW EVENT TYPE ---
#[derive(Clone, Debug)]
pub struct EditorSelectionRangeChanged {
    pub range: std::ops::Range<usize>,
}

impl<W: typst::World + typforge_core::IdeWorld + 'static> EventEmitter<EditorSelectionRangeChanged>
    for EditorPanel<W>
{
}
    ...
    EditorPanel<W>
{
    ...
    fn subscribe_to_editor_changes(
        ...
        // 2. Observe range changes and EMIT selection events
        cx.observe(editor_state, move |this_view, editor_entity, cx| {
            let content = editor_entity.read(cx).text().to_string();
            let cursor = editor_entity.read(cx).cursor();
            let range = editor_entity.read(cx).selected_range();

            this_view.update_ribbon_selection(&content, cursor, cx);
            cx.emit(EditorSelectionRangeChanged { range }); // <-- Notify of selection bounds change
        })
        .detach();
    }
}
