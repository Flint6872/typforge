//typforge0.0.1\crates\typforge\src\editor\code_editor.rs
...
impl Render for CodeEditor {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut input_container = div().id("input-container").size_full().child(input);
        ...
        v_flex()
            ...
            .child(div().flex_grow().child(input_container))
    }
}
...
impl IntoElement for CodeEditor {
    ...
    fn into_element(self) -> Self::Element {
        ...
        if let Some(listener) = self.on_mouse_move {
            input_container = input_container.on_mouse_move(listener);
        }
